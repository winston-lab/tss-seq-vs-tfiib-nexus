...
rule match_tfiib_to_tss:
    ...
    params:
        ...
        feature_distance = lambda wc: "feature_distance\t" if wc.category in ["intragenic", "antisense", "convergent", "divergent"] else "",
        drop_columns = lambda wc: "37,38,40" if wc.category in ["intragenic", "antisense", "convergent", "divergent"] else "36,37,39"
    shell: """
        ...
        cut --complement -f1-6,15,16,18,26,{params.drop_columns} | \
        cat <(echo -e "tss_chrom\ttss_start\ttss_end\ttss_name\ttss_score\ttss_strand\ttss_lfc\ttss_lfc_SE\ttss_FDR\ttss_expr_condition\ttss_expr_control\ttss_summit\tfeature_chrom\tfeature_start\tfeature_end\tfeature_name\tfeature_strand\t{params.feature_distance}tfiib_chrom\ttfiib_start\ttfiib_end\ttfiib_name\ttfiib_score\ttfiib_strand\ttfiib_lfc\ttfiib_lfc_SE\ttfiib_FDR\ttfiib_abundance_condition\ttfiib_abundance_control") - > {output}
        """
...
rule match_tss_to_tfiib:
    ...
    shell: """
        ...
        cut --complement -f1-6,15,16,18,26,{params.drop_columns} | \
        cat <(echo -e "tfiib_chrom\ttfiib_start\ttfiib_end\ttfiib_name\ttfiib_score\ttfiib_strand\ttfiib_lfc\ttfiib_lfc_SE\ttfiib_FDR\ttfiib_expr_condition\ttfiib_expr_control\ttfiib_summit\tfeature_chrom\tfeature_start\tfeature_end\tfeature_name\tfeature_strand\t{params.feature_distance}tss_chrom\ttss_start\ttss_end\ttss_name\ttss_score\ttss_strand\ttss_lfc\ttss_lfc_SE\ttss_FDR\ttss_expr_condition\ttss_expr_control") - > {output}
        """
