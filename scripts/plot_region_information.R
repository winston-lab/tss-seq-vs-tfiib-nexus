...
main = function(tfiib_rel_tss_path, tss_rel_tfiib_path, group, max_dist,
                rel_distances_out, tss_signal_violin_out, tss_signal_density_out,
    ...
    tfiib_rel_tss = read_tsv(tfiib_rel_tss_path,
                  col_names = c("chrom", "tss_start", "tss_end", "a", "tss_score", "tss_strand",
                                ...
                                "d", "tfiib_start", "tfiib_end", "e", "tfiib_coverage", "distance", "category")) %>%
        select(-c(a,b,c,d,e)) %>%
    ...
    tss_rel_tfiib = read_tsv(tss_rel_tfiib_path,
                             col_names = c("chrom", "tfiib_start", "tfiib_end", "a", "tfiib_coverage", "b",
                                "tss_start", "tss_end", "c", "tss_score", "tss_strand",
                                "d", "tss_pval", "tss_qval", "e", "tss_coverage", "distance")) %>%
        select(-c(a,b,c,d,e)) %>%
        mutate(distance = abs(distance)) %>%
        group_by(chrom, tfiib_start, tfiib_end) %>%
    ...
    tfiib_rel_tss_distances = ggplot(data = tfiib_rel_tss, aes(x=-distance+0.1)) +
        ...
        facet_grid(order~category) +
        ggtitle(group) +
        scale_x_continuous(limits = c(NA, 300),
                           name = "(-) displacement from TSS summit to TFIIB region (nt)") +
    ...
    tss_rel_tfiib_distances = ggplot(data = tss_rel_tfiib, aes(x=distance+0.1)) +
        ...
        facet_grid(order~.) +
        scale_x_continuous(limits = c(NA, 300),
                           name = "distance from TFIIB region to TSS summit (bp)") +
        scale_y_continuous(breaks = scales::pretty_breaks(2)) +
        theme_default
    ...
    tss_signal_group_match_violin = ggplot(data = tfiib_rel_tss_first,
        ...
        ggtitle(bquote(bold(.(group) ~ TSS ~ signal %+-% matching ~ TFIIB ~ region))) +
        theme_default +
    ...
    tss_rel_tfiib_counts = tss_rel_tfiib_first %>%
        count(match) %>%
        arrange(desc(match)) %>%
    ...
    tss_rel_tfiib_mosaic = ggplot(data = tss_rel_tfiib_counts) +
        ...
        scale_fill_ptol() +
        theme_void() +
        ggtitle(paste0(group, ": TSS-seq peak status around TFIIB regions")) +
        theme(text = element_text(size=12, color="black", face="bold"),
              plot.title = element_text(size=12, face="bold"))
    ...
    ggsave(tfiib_signal_density_out, tfiib_signal_group_match_density, width=14, height=6, units="cm")
    ggsave(tfiib_rel_tss_mosaic_out, tfiib_rel_tss_mosaic, width=18, height=9, units="cm")
    ggsave(tss_rel_tfiib_mosaic_out, tss_rel_tfiib_mosaic, width=8, height=9, units="cm")
}
...
main(tfiib_rel_tss_path = snakemake@input[["tfiib_rel_tss"]],
     ...
     rel_distances_out= snakemake@output[["signal_region_distances"]],
